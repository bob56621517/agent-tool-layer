...
- `BRAVE_API_KEY` —— Brave Search API 免费额度(2000 次/月)。填了则启用 braveapi 引擎(官方 API,稳定不被反爬);不填则该引擎不启用。
...
- **braveapi(官方 API)**:Brave Search API。设了 `BRAVE_API_KEY` 则启用(免费额度 2000 次/月),走官方 API 稳定、不被反爬;不设则不启用。
